...
// Native Terminal-Seite (aus Core-Hub portiert). Verbindet sich mit dem
// Plugin-WebSocket unter /app/terminal/ws (JSON {type:'data'|'resize'}).
...
export function TerminalPage() {
  ...
  const connect = useCallback((term: XTerm, fit: FitAddon) => {
    ...
    const ws = new WebSocket(`${proto}://${window.location.host}/app/terminal/ws`);
    wsRef.current = ws;
  ...
  return (
    <>
      <Topbar
        title={tt('Terminal')}
        subtitle={tt('Interaktive Server-Konsole')}
        actions={
      ...
      <main style={{ flex: 1, overflow: 'hidden', padding: '12px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div className="form-hint" style={{ flexShrink: 0 }}>
          ⚠️ {tt('Diese Konsole läuft mit erhöhten Rechten auf dem Server. Sei vorsichtig mit Befehlen, die das System verändern.')}
        </div>
      </main>
    </>
  );
