...
export function AntivirusPage() {
  ...
  const load = useCallback(async () => {
    setRefreshing(true);
    try { setAv(await get('/status')); } catch { /* */ } finally { setRefreshing(false); }
  }, []);

  useEffect(() => { void load(); return () => { if (pollRef.current) clearInterval(pollRef.current); }; }, [load]);
  useEffect(() => {
    if (av?.scan.running && !pollRef.current) pollRef.current = setInterval(load, 2500);
    else if (!av?.scan.running && pollRef.current) { clearInterval(pollRef.current); pollRef.current = null; }
  }, [av?.scan.running, load]);

  const act = async (key: string, fn: () => Promise<unknown>) => {
    setBusy(key);
    try { await fn(); await load(); } catch (err) { alert(err instanceof Error ? err.message : 'Fehler'); } finally { setBusy(''); }
  };
  const startScan = async () => {
    if (!scanPath.startsWith('/')) { alert(tt('Absoluter Pfad erforderlich')); return; }
    await act('scan', () => post('/scan', { path: scanPath, exclude: scanExclude.trim() || undefined }));
  };
  ...
  return (
    <>
      <Topbar title={tt('Virenschutz')} subtitle={av?.installed ? av.version || 'ClamAV' : undefined} onRefresh={load} refreshing={refreshing} />
      <main className="page">
        {!av ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-state__desc">{tt('ClamAV ist ein quelloffener Virenscanner für Linux. Mit einem Klick installieren:')}</div>
            <button className="btn btn--primary btn--sm" style={{ marginTop: 16 }} disabled={busy === 'install'} onClick={() => act('install', () => post('/install'))}>
              {busy === 'install' ? <span className="spinner" style={{ width: 12, height: 12 }} /> : <Download size={13} />} {tt('ClamAV installieren')}
            </button>
            <div className="empty-state__desc" style={{ marginTop: 16 }}>
              {tt('oder manuell')}: <code style={{ fontFamily: 'var(--font-mono)', fontSize: 12, background: 'var(--color-surface-sunken)', padding: '3px 7px', borderRadius: 5 }}>sudo apt install clamav clamav-daemon</code>
            </div>
          </div>
        ) : (
          <SortablePanels storageKey="antivirus" items={[
            { id: 'status', node: (
              <Panel title={tt('Status')} icon={<ShieldCheck size={15} />} subtitle={av.version} storageKey="av-status"
                actions={<div style={{ display: 'flex', gap: 4 }} onClick={(e) => e.stopPropagation()}>
                  <button className="btn btn--outline btn--sm" disabled={busy === 'update'} onClick={() => act('update', () => post('/update'))}>
                    {busy === 'update' ? <span className="spinner" style={{ width: 12, height: 12 }} /> : <RefreshCw size={13} />} {tt('Signaturen aktualisieren')}
                  </button>
                </div>}>
                <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'center', marginTop: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Switch checked={av.daemonActive} disabled={busy === 'daemon'} onChange={(v) => act('daemon', () => post('/daemon', { service: 'daemon', enable: v }))} />
                    <span style={{ fontSize: 12.5, fontWeight: 600 }}>{tt('Scan-Daemon')}</span>
                    <span className={`badge badge--${av.daemonActive ? 'running' : 'stopped'}`}><span className="badge__dot" />{av.daemonActive ? tt('aktiv') : tt('inaktiv')}</span>
                    {busy === 'daemon' && <span className="spinner" style={{ width: 12, height: 12 }} />}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Switch checked={av.freshclamActive} disabled={busy === 'freshclam'} onChange={(v) => act('freshclam', () => post('/daemon', { service: 'freshclam', enable: v }))} />
                    <span style={{ fontSize: 12.5, fontWeight: 600 }}>{tt('Auto-Updates')}</span>
                    <span className={`badge badge--${av.freshclamActive ? 'running' : 'stopped'}`}><span className="badge__dot" />{av.freshclamActive ? tt('aktiv') : tt('inaktiv')}</span>
                  ...
                  <span className={`badge badge--${av.defsAgeDays !== null && av.defsAgeDays <= 7 ? 'running' : 'restarting'}`} style={{ height: 24, padding: '0 10px' }}>
                    {tt('Signaturen')}: {av.defsAgeDays === null ? tt('unbekannt') : av.defsAgeDays === 0 ? tt('heute') : `${av.defsAgeDays} ${tt('Tage alt')}`}
                  </span>
                </div>
              </Panel>
            ) },
            { id: 'scan', node: (
              <Panel title={tt('Scan')} icon={<Search size={15} />} subtitle={s?.running ? tt('läuft…') : undefined} storageKey="av-scan">
                <div style={{ display: 'flex', gap: 8, marginTop: 10, marginBottom: 14, flexWrap: 'wrap' }}>
                  ...
                  <button className="btn btn--primary btn--sm" disabled={s?.running || busy === 'scan'} onClick={startScan}>
                    {s?.running || busy === 'scan' ? <span className="spinner" style={{ width: 12, height: 12 }} /> : <Play size={13} />} {tt('Scan starten')}
                  </button>
                </div>
                ...
                <div className="form-group" style={{ marginBottom: 14 }}>
                  <label className="form-label">{tt('Ordner ausschließen (optional, mehrere mit Komma)')}</label>
                  <input className="input input--rect" value={scanExclude} onChange={(e) => setScanExclude(e.target.value)} placeholder="/home/dirk/Downloads, /var/lib/docker" style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }} disabled={s?.running} />
                </div>
                {s?.running && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--color-accent)', fontSize: 13 }}>
                    <span className="spinner" style={{ width: 14, height: 14 }} />
                    {tt('Scanne')} {s.path} … {s.scanned > 0 && `${s.scanned} ${tt('Dateien geprüft')}`}{s.infectedCount > 0 && ` · ${s.infectedCount} ${tt('Funde')}`}
                  </div>
                )}
                {lastClean && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--color-success)', fontSize: 14, fontWeight: 600 }}>
                    <CheckCircle2 size={18} /> {tt('Keine Bedrohungen gefunden in')} {s.path} ({s.scanned} {tt('Dateien')})
                  </div>
                )}
                  ...
                  <div className="card" style={{ borderColor: 'var(--color-error)' }}>
                    <div className="card-body">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--color-error)', fontWeight: 700, marginBottom: 8 }}>
                        <AlertOctagon size={18} /> {s.infectedCount} {tt('Bedrohung(en) gefunden!')}
                      </div>
