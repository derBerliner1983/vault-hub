...
// Native SMB-Freigaben-Seite (aus Core-Hub portiert). Spricht das Plugin-Backend
// unter /app/shares/api statt der Core-Hub-API.
const API = '/app/shares/api';
      ...
      footer={
        <>
          ...
          <button className="btn btn--primary btn--sm" onClick={save} disabled={loading}>
            {loading && <span className="spinner" style={{ width: 12, height: 12 }} />} {tt('Freigabe erstellen')}
          </button>
        </>
      }
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
        {check('readOnly', tt('Nur Lesen'), tt('Schreibzugriff verbieten'))}
        {check('guestOk', tt('Gastzugriff'), tt('ohne Passwort zugänglich'))}
        {check('browseable', tt('Sichtbar'), tt('in der Netzwerkumgebung anzeigen'))}
      </div>
    </Modal>
  );
}
...
export function SharesPage() {
  ...
  const svc = async (action: 'start' | 'stop' | 'restart') => {
    setBusy(true);
    try { await post('/service', { action }); setTimeout(load, 800); }
  ...
  const remove = async (name: string) => {
    ...
    try { await del('/' + encodeURIComponent(name)); await load(); }
    catch (err) { alert(err instanceof Error ? err.message : 'Fehler'); }
  };

  const fwLabel = firewallOpen ? tt('LAN offen') : tt('geblockt');
  ...
  const topbarSubtitle = available
    ? `Samba ${running ? tt('läuft') : tt('gestoppt')} · ${fwLabel}`
    : undefined;

  return (
    <>
      <Topbar
        title={tt('SMB-Freigaben')}
          ...
          <>
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                {shares.length === 0 ? (
                  <span>
                    <strong>{tt('Samba automatisch deaktiviert')}</strong> – {tt('Firewall blockiert Ports 137–139, 445.')}
                    {' '}{tt('Samba startet automatisch, wenn du eine Freigabe hinzufügst.')}
                  </span>
                ) : firewallOpen ? (
                  <span>
                    <strong>{tt('Samba aktiv – nur LAN')}</strong> – {tt('Ports 137–139, 445 sind nur aus dem lokalen Netzwerk erreichbar.')}
                  </span>
                ) : (
                  <span>
                    <strong>{tt('Samba läuft, aber Firewall blockiert')}</strong> – {tt('Ports sind noch nicht freigegeben.')}
                  </span>
                )}
              </div>
            ...
            <Panel
              ...
              subtitle={`${shares.length} ${tt('aktiv')}`}
              storageKey="shares"
              actions={
                      ...
                      <tr key={s.name}>
                        ...
                        <td>
                          <span className={`badge badge--${s.readOnly ? 'stopped' : 'running'}`}>
                            <span className="badge__dot" />{s.readOnly ? tt('Nur Lesen') : tt('Lesen/Schreiben')}
                          </span>
                        </td>
                        <td className="text-muted text-sm">
                          {s.guestOk ? tt('Gast') + ' · ' : ''}{s.browseable ? tt('sichtbar') : tt('versteckt')}
                        </td>
                        <td>
