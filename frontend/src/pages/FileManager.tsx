...
import {
  Folder, File, FileText, ChevronRight, Home, Trash2, Download, Upload,
...
export function FileManager() {
  ...
  const load = useCallback(async (dir: string) => {
    ...
    setSelected(null);
    try {
      const data = await freq<{ path: string; entries: Entry[]; parent: string | null }>(`/api/files?path=${encodeURIComponent(dir)}`);
      setCwd(data.path);
  ...
  const openEdit = async (entry: Entry) => {
    setError('');
    try {
      const data = await freq<{ content: string }>(`/api/files/read?path=${encodeURIComponent(entry.path)}`);
  ...
  const saveEdit = async () => {
    ...
    setEditSaving(true);
    try {
      await freq('/api/files/write', { method: 'POST', body: JSON.stringify({ path: editPath, content: editContent }) });
      setEditPath(null);
      void load(cwd);
    } catch (e) { setError(e instanceof Error ? e.message : 'Fehler'); }
  ...
  const deleteEntry = async (entry: Entry) => {
    ...
    try {
      await freq(`/api/files?path=${encodeURIComponent(entry.path)}`, { method: 'DELETE' });
      void load(cwd);
    } catch (e) { setError(e instanceof Error ? e.message : 'Fehler'); }
  };
  ...
  const createFolder = async () => {
    ...
    try {
      const newPath = cwd.replace(/\/$/, '') + '/' + newFolderName.trim();
      await freq('/api/files/mkdir', { method: 'POST', body: JSON.stringify({ path: newPath }) });
      setNewFolderName('');
  ...
  const applyChmod = async () => {
    if (!chmodPath) return;
    try {
      await freq('/api/files/chmod', { method: 'POST', body: JSON.stringify({ path: chmodPath, mode: chmodMode }) });
      setChmodPath(null);
      void load(cwd);
    } catch (e) { setError(e instanceof Error ? e.message : 'Fehler'); }
  };
  ...
  const applyRename = async () => {
    ...
    const to = dir + '/' + renameTo.trim();
    try {
      await freq('/api/files/rename', { method: 'POST', body: JSON.stringify({ from: renamePath, to }) });
      setRenamePath(null);
      void load(cwd);
    } catch (e) { setError(e instanceof Error ? e.message : 'Fehler'); }
  };
  ...
  const uploadFiles = async (files: FileList | null) => {
    ...
    try {
      const res = await fetch(`/api/files/upload?path=${encodeURIComponent(cwd)}`, { method: 'POST', body: form, credentials: 'include' });
      if (!res.ok) { const b = await res.json().catch(() => ({})); throw new Error((b as { error?: string }).error ?? `HTTP ${res.status}`); }
      void load(cwd);
    } catch (e) { setError(e instanceof Error ? e.message : 'Fehler'); }
  };
      ...
      <Topbar
        ...
        actions={
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button className="btn btn--ghost btn--sm" onClick={() => setShowNewFolder(f => !f)}>
      ...
      <main className="page">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 10, fontSize: 13, flexWrap: 'wrap' }}>
          ...
          {crumbs.map((seg, i) => {
            ...
            return (
              <span key={to} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <ChevronRight size={11} style={{ color: 'var(--color-faint)' }} />
                <button className="btn btn--ghost btn--sm" style={{ padding: '1px 6px', fontSize: 12 }} onClick={() => navigate(to)}>{seg}</button>
              </span>
            );
          })}
        </div>
            ...
            <table className="dtable">
              ...
              <tbody>
                ...
                {entries.map(entry => {
                  ...
                  return (
                    <tr key={entry.path} style={{ background: sel ? 'var(--color-accent-subtle, rgba(99,102,241,.08))' : undefined }}
                      onClick={() => setSelected(sel ? null : entry.path)}>
                      <td style={{ paddingRight: 0 }}><FileIcon entry={entry} /></td>
                      <td>
                        <button
                          className="btn btn--ghost btn--sm"
                          style={{ fontWeight: entry.isDir ? 600 : 400, textAlign: 'left', padding: '0 4px', maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                          onClick={e => { e.stopPropagation(); entry.isDir ? navigate(entry.path) : (isText ? void openEdit(entry) : undefined); }}
                          title={entry.path}
                        >
                          {entry.name}{entry.isSymlink ? ' →' : ''}
                        </button>
                      </td>
                      <td className="dtable__mono" style={{ fontSize: 11.5 }}>
                        <span title={`${entry.permissions} (${permStr(entry.mode)})`}>{permStr(entry.mode)}</span>
                      </td>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 200, display: 'flex', flexDirection: 'column', padding: 24 }}>
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 12, flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', maxHeight: '90vh' }}>
              ...
              <textarea
                ...
                style={{ flex: 1, resize: 'none', border: 'none', outline: 'none', background: 'var(--color-input)', fontFamily: 'monospace', fontSize: 13, padding: '12px 16px', color: 'var(--color-text)', lineHeight: 1.6 }}
                spellCheck={false}
              />
            </div>
          </div>
...
function ChmodEditor({ mode, onChange }: { mode: string; onChange: (m: string) => void }) {
  ...
  const rows = [
    { label: 'Besitzer', r: 0o400, w: 0o200, x: 0o100 },
    { label: 'Gruppe', r: 0o040, w: 0o020, x: 0o010 },
    { label: 'Andere', r: 0o004, w: 0o002, x: 0o001 },
  ];
  ...
  return (
    <div>
      <table style={{ width: '100%', fontSize: 13 }}>
        <thead>
          <tr>
            <th style={{ textAlign: 'left', paddingBottom: 6, color: 'var(--color-muted)', fontWeight: 400 }}></th>
            {['Lesen', 'Schreiben', 'Ausführen'].map(h => <th key={h} style={{ textAlign: 'center', color: 'var(--color-muted)', fontWeight: 400 }}>{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map(row => (
            <tr key={row.label}>
              ...
              {[['r', row.r], ['w', row.w], ['x', row.x]].map(([, pos]) => (
                <td key={String(pos)} style={{ textAlign: 'center', paddingBottom: 6 }}>
                  <input type="checkbox" checked={bit(Number(pos))} onChange={() => toggle(Number(pos))} />
                </td>
              ))}
            </tr>
          ))}
      ...
      <div style={{ marginTop: 8, fontSize: 12, color: 'var(--color-muted)' }}>
        Octal: <code style={{ fontFamily: 'monospace', background: 'var(--color-input)', padding: '1px 6px', borderRadius: 4 }}>{mode}</code>
        {' = '}
        <code style={{ fontFamily: 'monospace', background: 'var(--color-input)', padding: '1px 6px', borderRadius: 4 }}>{(() => { const o = parseInt(mode, 8) || 0; const sym = (n: number) => [(n&4)?'r':'-',(n&2)?'w':'-',(n&1)?'x':'-'].join(''); return sym(o>>6)+sym((o>>3)&7)+sym(o&7); })()}</code>
      </div>
    </div>
  );
}
